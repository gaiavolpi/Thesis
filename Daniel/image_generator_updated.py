...
file_name = 'CNN_Images_Run31_snr_reduced_m1_m2_num_reduced'

root_dir = '/data/danibelt'
...
# source paramters
mass1_tr = [18, 20, 24, 26, 28, 32, 34, 36, 40, 42, 46, 50]
mass2_tr = [19, 21, 25, 27, 29, 33, 35, 37, 41, 43, 47, 51]

#distance_tr = [500,1000] # in Mpc
distance_red_tr = [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 0.75, 1, 1.5] # in redshift
...
# source paramters
mass1_te = [15, 34, 53, 13, 38, 40]
mass2_te = [12, 16, 21, 22, 24, 61]

#distance_te = [500] # in Mpc
distance_red_te = [0.003, 0.04, 0.09, 0.2, 0.7] # in redshift
...
def injection_by_hand(strain, signal):
    ...
    loc = np.random.randint(2 * 2048, dif - 2 * 2048)
    ...
    scale = np.mean(abs(np.array(strain))) / np.mean(abs(np.array(signal))) * (1/5)
            ...
            for inc in inclination:
                for di in distance:
                    for i in range(len(ra)):
                        ...
                        #ra_aux = float("{:.3f}".format(float(np.squeeze(ra[i])))) # to avoid too much floating decimals
                        #dec_aux = float("{:.3f}".format(float(np.squeeze(dec[i]))))
                        ...
                        #scale = abs(float(np.random.randn(1,1) * 0.0001 + 0))
                        ...
                        plt.savefig(dir_name + '/mass1-{}_mass2-{}_sample{}.png'.format(M1, M2, it), pad_inches=0.0, bbox_inches='tight')
                        plt.close(fig)
