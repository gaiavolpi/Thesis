...
file_name = 'CNN_Images_Run36_m1_m2_uniform_Run2'
...
# source paramters
mass1_tr = np.squeeze(np.random.uniform(low=15, high=90, size=(1,100)))
mass2_tr = np.squeeze(np.random.uniform(low=15, high=90, size=(1,100)))

#distance_tr = [500,1000] # in Mpc
distance_red_tr = np.squeeze(np.random.uniform(low=0.001, high=1.5, size=(1,4))) # in redshift
...
# orbit inclination
inclination_tr = np.squeeze(np.random.uniform(low=0, high=1, size=(1,4)))

# sky location of the source (randomly) --> for the moment 3 locations
ra_tr = np.squeeze(np.random.uniform(low=0, high=2*np.pi, size = (1, 4)))
dec_tr = np.squeeze(np.random.uniform(low=0, high=np.pi, size = (1, 4)))
...
# source paramters
mass1_te = np.squeeze(np.random.uniform(low=15, high=90, size=(1,50)))
mass2_te = np.squeeze(np.random.uniform(low=15, high=90, size=(1,50)))

#distance_te = [500,1000] # in Mpc
distance_red_te = np.squeeze(np.random.uniform(low=0.001, high=1.5, size=(1,3))) # in redshift
#distance_te = np.array(astr_cosm.Planck15.luminosity_distance(np.array(distance_red_tr))) # convertion to Mpc with Planck18 cosmology

# orbit inclination
inclination_te = np.squeeze(np.random.uniform(low=0, high=1, size=(1,2)))

# sky location of the source (randomly) --> for the moment 3 locations
ra_te = np.squeeze(np.random.uniform(low=0, high=2*np.pi, size = (1, 2)))
dec_te = np.squeeze(np.random.uniform(low=0, high=np.pi, size = (1, 2)))
...
delta_t = 1.0 / 2048
time_length = 2.5  #  seconds of data, since longest GW signal lasts 2 sec (m1=m2=10) in this sense we reduce computational resources
...
def injection_by_hand(strain, signal):
    ...
    scale = np.squeeze(np.random.uniform(low=0.055,high=0.8,size=(1,1)))
